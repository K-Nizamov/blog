...
function PostsList({ posts,setId,setImg }) {
        ...
        <ul className="posts-list">
            {posts.map(x => {
                return (<li className="post" key={x.id}>
                    ...
                    <Link to={x.slug}><button className="read-more-btn" onClick={()=>setId(x.id)} onClick={()=> setImg(x["_embedded"]["wp:featuredmedia"][0]["source_url"])} >Read More ...</button></Link>
                </li>)
            })}
        </ul>
    );
